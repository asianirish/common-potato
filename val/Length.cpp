...
QString Length::errorMessage() const
{
    ...
    if (_minLength.isValid() && !_maxLength.isValid()) {
        return QString(tr("the string must contain more than %n characters", "", _minLength.toInt()));
    }

    if (!_minLength.isValid() && _maxLength.isValid()) {
        return QString(tr("the string must contain less than %n characters", "", _maxLength.toInt()));
    }
    ...
    return QString(tr("wrong length validator format"));
}
